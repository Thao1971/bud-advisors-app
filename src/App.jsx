...
// REGLA 1: Saneamos el ID de la app para evitar errores de rutas (segmentos pares)
// Usamos replace(/\//g, '_') para asegurar que no haya slashes internos que añadan segmentos
...
const formatPercent = (v) => (!v || isNaN(v)) ? '-' : new Intl.NumberFormat('es-ES', { style: 'percent', maximumFractionDigits: 1 }).format(v);

export default function App() {
  const [data, setData] = useState([]);
  const [user, setUser] = useState(null);
  const [status, setStatus] = useState({ type: 'info', msg: 'Iniciando conexión...' });
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (u) {
        setStatus({ type: 'success', msg: 'SISTEMA ONLINE - Cloud Sync Activo' });
      } else {
        setLoading(false);
      }
    });
  ...
  // 2. Sincronización en tiempo real (REGLA 1 y 3)
  useEffect(() => {
    if (!user || !db) return;
    
    // Ruta estricta de 5 segmentos: artifacts/{appId}/public/data/companies
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      setStatus({ type: 'error', msg: err.code === 'permission-denied' ? 'Firestore: Permisos denegados (Revisa las reglas).' : err.message });
      setLoading(false);
    });
  ...
  // 3. Lógica de subida CSV
  const handleUpload = (e) => {
    const file = e.target.files[0];
    if (!file || !db || !user) return;
    setUploading(true);
    setStatus({ type: 'info', msg: 'Actualizando base de datos en la nube...' });
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (obj['CIF EMPRESA']) {
            ...
            // REGLA 1: Document path must have an even number of segments (6)
            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'companies', id), obj);
          }
        }
        setStatus({ type: 'success', msg: '¡Base de datos sincronizada correctamente!' });
      } catch (err) { setStatus({ type: 'error', msg: `Error en subida: ${err.message}` }); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <nav className="bg-black text-white p-6 border-b-4 border-yellow-400 flex justify-between items-center sticky top-0 z-50 shadow-2xl">
        ...
        <label className={`bg-yellow-400 hover:bg-yellow-300 text-black px-6 py-2.5 font-black text-xs uppercase tracking-widest cursor-pointer transition-all flex items-center gap-2 shadow-lg ${uploading ? 'opacity-50' : ''}`}>
          <UploadIcon />
      ...
      <main className="max-w-7xl mx-auto p-8">
        ...
        <div className="bg-white p-8 shadow-xl mb-12 border-t-8 border-black flex flex-col gap-8">
          <div className="flex items-center gap-4 border-b-2 border-gray-100 pb-4 group">
            ...
            <input 
              className="w-full outline-none font-bold text-xl placeholder-gray-300 bg-transparent"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Especialidad / Subcategoría</span>
              <div className="relative">
        ...
        {/* LISTADO */}
        {loading ? (
          <div className="text-center py-20 animate-pulse font-black text-gray-400 uppercase tracking-widest">Sincronizando registros...</div>
        ) : (
        ...
        {filtered.length === 0 && !loading && (
          <div className="text-center py-32 bg-white border-4 border-dashed border-gray-100 rounded-lg">
            <SearchIcon />
            <p className="mt-4 font-black text-gray-300 uppercase tracking-widest text-lg">No hay coincidencias en el registro</p>
          </div>
        )}
      </main>
          ...
          <div className="bg-white w-full max-w-5xl max-h-[92vh] overflow-y-auto shadow-2xl border-t-[12px] border-yellow-400 animate-in fade-in zoom-in duration-300">
            <div className="p-10 md:p-16 text-slate-900">
              <div className="flex justify-between items-start mb-12">
                <div>
                  <div className="flex items-center gap-2 mb-2 text-yellow-600 font-black text-[10px] uppercase tracking-widest">
                    <CloudIcon /> EXPEDIENTE ESTRATÉGICO
                  </div>
                  <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-tight mb-2">{String(selectedCompany['DENOMINACIÓN SOCIAL'])}</h2>
                  <p className="text-gray-400 font-mono text-sm uppercase">CIF: {String(selectedCompany['CIF EMPRESA'])} | {String(selectedCompany['CATEGORÍA'])} / {String(selectedCompany['SUBCATEGORÍA'])}</p>
                </div>
                <button onClick={() => setSelectedCompany(null)} className="p-3 hover:bg-gray-100 rounded-full text-black border-2 border-gray-100 transition-colors shadow-sm"><XIcon /></button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                ...
                <div className="space-y-6">
                  <h4 className="text-lg font-black uppercase border-b-4 border-black pb-2 flex justify-between items-end">
                    ...
                    <span className="text-[10px] text-gray-400 tracking-widest font-bold">CIFRAS EN EUR</span>
                  </h4>
                  <div className="space-y-3 font-medium text-sm">
                    <div className="flex justify-between p-3 bg-gray-50 font-bold border-l-4 border-black">
                      <span>(+) Ventas Netas</span>
                      <span className="tabular-nums font-black">{formatCurrency(selectedCompany['IMPORTEN NETO DE LA CIFRA DE NEGOCIO'])}</span>
                    </div>
                    <div className="flex justify-between p-3 text-red-600 italic">
                      <span>(-) Aprovisionamientos</span>
                      <span className="tabular-nums">{formatCurrency(selectedCompany['APROVISIONAMIENTOS'])}</span>
                    </div>
                    <div className="flex justify-between p-3 text-red-600 italic">
                      <span>(-) Gastos de Personal</span>
                      <span className="tabular-nums">{formatCurrency(selectedCompany['GASTOS DE PERSONAL'])}</span>
                    </div>
                    <div className="flex justify-between p-3 text-red-600 italic border-b-2">
                      <span>(-) Otros Gastos de Explotación</span>
                      <span className="tabular-nums">{formatCurrency(selectedCompany['GASTOS DE EXPLOTACIÓN Y OTROS GASTOS DE EXPLOTACIÓN'])}</span>
                    </div>
                    <div className="flex justify-between p-3 bg-yellow-400/10 font-black text-lg border-l-4 border-yellow-400">
                      <span>(=) EBITDA</span>
                      <span className="text-yellow-600 tabular-nums">{formatCurrency(selectedCompany['EBITDA'])}</span>
                    </div>
                    <div className="flex justify-between p-3 italic opacity-60">
                      <span>Amortizaciones e Intereses</span>
                      <span className="tabular-nums">{formatCurrency((selectedCompany['AMORTIZACIONES'] || 0) + (selectedCompany['INTERESES'] || 0))}</span>
                    </div>
                    <div className="flex justify-between p-3 border-t-4 border-black font-black text-2xl mt-4 bg-gray-50">
                      <span>Resultado Neto</span>
                      <span className="tabular-nums">{formatCurrency(selectedCompany['RESULTADO DEL EJERCICIO'])}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="bg-black text-white p-8 border-l-8 border-yellow-400 shadow-xl">
                    <div className="flex items-center gap-2 mb-6 text-yellow-400">
                       <CloudIcon />
                       <h5 className="text-[10px] font-black uppercase tracking-widest">Recursos y Eficiencia</h5>
                    </div>
                    <div className="grid grid-cols-2 gap-8">
                      <div>
                        <span className="text-3xl font-black block leading-none">{selectedCompany['NÚMERO MEDIO DE EMPLEADOS'] || '-'}</span>
                        <span className="text-[10px] uppercase font-bold text-gray-400 mt-2 block tracking-widest">Consultores</span>
                      </div>
                      <div>
                        <span className="text-xl font-black block leading-none tabular-nums">
                          {formatCurrency((selectedCompany['IMPORTEN NETO DE LA CIFRA DE NEGOCIO'] || 0) / (selectedCompany['NÚMERO MEDIO DE EMPLEADOS'] || 1))}
                        </span>
                        <span className="text-[10px] uppercase font-bold text-gray-400 mt-2 block tracking-widest">Ventas / Pax</span>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-100 p-8 border-l-8 border-slate-300">
                    <h5 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4">Actividad Registrada</h5>
                    <p className="text-sm leading-relaxed italic font-serif text-slate-600 leading-relaxed">
                      "{String(selectedCompany['OBJETO SOCIAL'] || 'Descripción no disponible en el último cierre de ejercicio.')}"
                    </p>
                  </div>
                  <div className="p-4 border-2 border-dashed border-gray-200 text-center uppercase text-[10px] font-black text-gray-400 tracking-widest">
                    MARGEN EBITDA: {formatPercent((selectedCompany['EBITDA'] || 0) / (selectedCompany['IMPORTEN NETO DE LA CIFRA DE NEGOCIO'] || 1))}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-16 flex justify-center">
                <button 
                  ...
                  className="bg-black text-white px-20 py-5 font-black uppercase tracking-widest text-sm hover:bg-yellow-400 hover:text-black transition-all shadow-2xl active:scale-95"
                >
