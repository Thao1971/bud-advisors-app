...
import { 
  ...
  Target, Briefcase, DollarSign
} from 'lucide-react';
import { initializeApp } from 'firebase/app';
import { getAuth, signInAnonymously, onAuthStateChanged } from 'firebase/auth';
import { getFirestore, collection, doc, setDoc, onSnapshot } from 'firebase/firestore';

// --- SISTEMA DE CONFIGURACIÓN ULTRA-SEGURO ---
const getFirebaseConfig = () => {
  ...
  if (!configString && typeof __firebase_config !== 'undefined' && __firebase_config) {
    configString = __firebase_config;
  }

  if (!configString) return null;
  if (typeof configString === 'object') return configString;

  try {
    const cleanJson = configString.trim().replace(/[\u200B-\u200D\uFEFF]/g, '');
    return JSON.parse(cleanJson);
  } catch (e) {
    console.error("Configuración corrupta:", e);
    return null;
  }
};
...
const appId = "bud_market_intelligence_final"; 
...
export default function App() {
  ...
  const [status, setStatus] = useState({ type: 'info', msg: 'Analizando conexión con Google Cloud...' });
  ...
  useEffect(() => {
    if (!auth) {
      setStatus({ type: 'error', msg: 'ERROR: Configuración no detectada. Revisa Netlify.' });
      setLoading(false);
      return;
    }
    signInAnonymously(auth).catch(err => {
      setStatus({ type: 'error', msg: `ERROR DE CONEXIÓN: ${err.message}` });
      setLoading(false);
    });
  ...
  // 3. CARGA DE DATOS CSV MEJORADA (Soporte para punto y coma y limpieza de números)
  const handleUpload = (e) => {
    const file = e.target.files[0];
    if (!file || !db || !user) return;
    
    setUploading(true);
    setStatus({ type: 'info', msg: 'Sincronizando registros con la nube...' });
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (lines.length < 2) throw new Error("CSV sin datos válidos");
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          headers.forEach((h, idx) => {
            ...
            // Limpiador de números: elimina €, espacios, y convierte coma decimal a punto
      ...
      <nav className="bg-black text-white p-6 border-b-4 border-yellow-400 sticky top-0 z-50 shadow-2xl">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="bg-yellow-400 p-2 rounded-sm animate-pulse">
      ...
      <main className="max-w-7xl mx-auto p-8">
        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6 border-b-2 border-black pb-2">
            ...
            <h2 className="text-sm font-black uppercase tracking-widest italic">Análisis Global de Mercado</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-black text-white p-8 border-l-8 border-yellow-400 shadow-xl flex flex-col justify-center">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Entidades Totales</span>
              <div className="flex items-baseline gap-2">
                <span className="text-6xl font-black leading-none">{data.length}</span>
                <span className="text-xs font-bold text-yellow-400 uppercase">Unidades</span>
              </div>
            </div>
            ...
            {Object.entries(stats.catCounts).slice(0, 2).map(([cat, count]) => (
              <div key={cat} className="bg-white p-8 border-l-8 border-gray-200 shadow-lg group hover:border-yellow-400 transition-all">
                ...
                <span className="text-3xl font-black block leading-none">{count} <span className="text-xs text-gray-400 font-bold tracking-widest italic">Agencias</span></span>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="flex gap-6 overflow-x-auto pb-8 snap-x scrollbar-hide">
            {topTen.map((c, i) => (
              ...
              >
                ...
                <span className="text-[9px] font-black bg-black text-white px-2.5 py-1 uppercase tracking-widest mb-4 inline-block italic">
                  {c['CATEGORÍA']}
                </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="flex flex-col gap-3">
              <span className="text-[11px] font-black text-gray-400 uppercase tracking-[0.4em] flex items-center gap-2">
                <Filter className="w-3.5 h-3.5" /> Sector de Negocio
              </span>
              <select 
        ...
        {loading ? (
          <div className="text-center py-40 animate-pulse flex flex-col items-center gap-8">
            ...
            <span className="font-black text-gray-300 uppercase tracking-[0.6em] text-2xl italic">Sincronizando registros...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {filtered.map((c, i) => (
              ...
              >
                ...
                <p className="text-gray-400 text-[12px] font-mono mb-12 flex items-center gap-2 border-b border-gray-50 pb-4 italic text-xs uppercase tracking-widest">
                  CIF: {c['CIF EMPRESA']}
                </p>
      ...
      {selectedCompany && (
        <div className="fixed inset-0 bg-black/98 backdrop-blur-3xl z-[100] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-6xl my-auto shadow-2xl border-t-[20px] border-yellow-400 animate-in fade-in zoom-in duration-500 rounded-sm">
            <div className="p-12 md:p-24 text-slate-900">
              <div className="flex justify-between items-start mb-20">
                <div className="flex-1">
                  <span className="bg-black text-yellow-400 text-[12px] font-black px-5 py-2 uppercase tracking-[0.4em] shadow-xl italic mb-8 inline-block">EXPEDIENTE ESTRATÉGICO</span>
                  <h2 className="text-6xl md:text-8xl font-black tracking-tighter uppercase leading-none mb-8 italic">{selectedCompany['DENOMINACIÓN SOCIAL']}</h2>
                  <div className="flex flex-wrap gap-12 text-gray-400 font-mono text-sm border-l-4 border-black pl-8 uppercase">
                    <div>CIF: <span className="text-black font-black text-2xl">{selectedCompany['CIF EMPRESA']}</span></div>
                    <div>Sector: <span className="text-black font-black text-2xl">{selectedCompany['CATEGORÍA']}</span></div>
                    <div>Ejercicio: <span className="text-yellow-600 font-black text-2xl italic tracking-widest">{selectedCompany['EJERCICIO']}</span></div>
                  </div>
                </div>
                <button onClick={() => setSelectedCompany(null)} className="p-6 border-4 border-gray-100 rounded-full hover:bg-gray-100 transition-all text-black hover:rotate-90 group"><X className="w-12 h-12" /></button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-24">
                <div className="space-y-12">
                  <h4 className="text-3xl font-black uppercase border-b-[10px] border-black pb-6 flex justify-between items-end italic">Estructura P&L <span className="text-[12px] text-gray-400 tracking-[0.4em] font-bold uppercase italic">EUR</span></h4>
                  <div className="space-y-6">
                    <div className="flex justify-between p-8 bg-gray-50 border-l-[12px] border-black hover:bg-yellow-50 shadow-md">
                      <span className="font-black text-xl uppercase italic tracking-tighter">Ventas Netas</span>
                      <span className="text-4xl font-black tabular-nums">{formatCurrency(selectedCompany['IMPORTEN NETO DE LA CIFRA DE NEGOCIO'])}</span>
                    </div>
                    <div className="flex justify-between px-8 py-5 text-red-600 border-b-2 border-gray-50 italic font-bold uppercase text-xs"><span>(-) Gastos de Personal</span><span className="text-xl tabular-nums">{formatCurrency(selectedCompany['GASTOS DE PERSONAL'])}</span></div>
                    <div className="flex justify-between p-10 bg-yellow-400/10 border-l-[12px] border-yellow-400 mt-10 shadow-inner">
                      ...
                      <span className="text-4xl font-black text-yellow-600 tabular-nums">{formatCurrency(selectedCompany['EBITDA'])}</span>
                    </div>
                ...
                <div className="space-y-16">
                  <div className="bg-black text-white p-14 border-l-[15px] border-yellow-400 shadow-2xl relative overflow-hidden group rounded-sm">
                    <h5 className="text-[12px] font-black uppercase tracking-[0.5em] text-yellow-400 mb-12 flex items-center gap-3"><CheckCircle2 className="w-4 h-4" /> Eficiencia Humana</h5>
                    <div className="grid grid-cols-2 gap-20 relative z-10">
                      <div><span className="text-8xl font-black block leading-none mb-4 tracking-tighter italic">{selectedCompany['NÚMERO MEDIO DE EMPLEADOS'] || '-'}</span><span className="text-[12px] uppercase font-bold text-gray-400 tracking-[0.3em] block">Pax Plantilla</span></div>
                      <div className="border-l border-white/10 pl-10">
                        <span className="text-4xl font-black block leading-none mb-4 tracking-tighter text-yellow-400 italic">{formatCurrency((selectedCompany['IMPORTEN NETO DE LA CIFRA DE NEGOCIO'] || 0) / (selectedCompany['NÚMERO MEDIO DE EMPLEADOS'] || 1))}</span>
                        <span className="text-[12px] uppercase font-bold text-gray-400 tracking-[0.3em] block">Prod / Pax</span>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-100 p-14 border-l-[15px] border-slate-300 shadow-md">
                    <h5 className="text-[12px] font-black uppercase tracking-[0.5em] text-gray-400 mb-10 flex items-center gap-3"><Briefcase className="w-5 h-5 text-gray-400" /> Objeto Social Registrado</h5>
                    <p className="text-2xl leading-relaxed italic font-serif text-slate-700 font-medium">"{String(selectedCompany['OBJETO SOCIAL'] || 'Descripción no disponible.')}"</p>
                  </div>
                </div>
              </div>

              <div className="mt-28 flex justify-center">
                <button onClick={() => setSelectedCompany(null)} className="bg-black text-white px-40 py-10 font-black uppercase tracking-[0.6em] text-sm hover:bg-yellow-400 hover:text-black transition-all shadow-2xl active:scale-95 border-b-[15px] border-yellow-600 rounded-sm">Cerrar Análisis de Inteligencia</button>
              </div>
            </div>
          </div>
        </div>
